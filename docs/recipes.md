...
```python
after = workflow.run(copy.deepcopy(store))
```
...
```python
workflow.run(store, on_step=lambda step, ops, store: log.info("%s: %d ops", step.name, len(ops)))
```
...
Cheaper still: put it in a test. The workflow's shape is static, so a wrong order is a unit-test failure,
...
```python
workflow.run(store, copy_inputs=False)
```
